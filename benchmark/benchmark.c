...
static int write_naive_file(const char *path, const long *timestamps, const double *values, int n) {
    ...
    for (int i = 0; i < n; i++) {
        if (fwrite(&timestamps[i], 8, 1, f) != 1) { fclose(f); return -1; }
        if (fwrite(&values[i], 8, 1, f) != 1) { fclose(f); return -1; }
    }
...
int main(int argc, char **argv) {
    ...
    double last_val[10];
    for (int m = 0; m < metrics; m++) last_val[m] = 50.0 + (double)m;

    int idx = 0;
    for (int i = 0; i < points_per_metric; i++) {
        for (int m = 0; m < metrics; m++) {
            long ts = base_ts + i;
            double v = last_val[m] + (rand_drift() * 0.02);
            if (i % 5 == 0) v = last_val[m];
            last_val[m] = v;
